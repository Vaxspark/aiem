    ...
    match &s.transport {
        McpTransport::Stdio { command, args, env, cwd } => {
            ...
            if !env.is_empty() {
                ...
                for (k, v) in env { e.insert(k.clone(), Value::String(v.clone())); }
...
pub fn read(project_root: Option<&Path>) -> Result<Vec<McpServer>> {
    ...
    if let Some(Value::Table(t)) = root.get("mcp_servers") {
        for (name, val) in t {
            ...
            let command = st.get("command").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let args = st
                .get("args")
            ...
            let env = st
                ...
                .map(|e| e.iter().filter_map(|(k, v)| v.as_str().map(|s| (k.clone(), s.to_string()))).collect::<BTreeMap<_, _>>())
                .unwrap_or_default();
