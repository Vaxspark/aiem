//! Claude Code adapter: reads/writes `~/.claude.json`.
//!
//! Claude Code's global config stores MCP servers under the top-level
//! `mcpServers` object. Each entry is a JSON object with fields `command`,
//! `args`, `env`, or (for HTTP / SSE) `type` + `url`.
//!
//! Project scope uses `.mcp.json` at the project root.
...
fn server_to_json(s: &McpServer) -> Value {
    match &s.transport {
        McpTransport::Stdio { command, args, env, cwd, bundle: _ } => {
            ...
            if !args.is_empty() { obj.insert("args".into(), json!(args)); }
            if !env.is_empty() { obj.insert("env".into(), json!(env)); }
            if let Some(cwd) = cwd { obj.insert("cwd".into(), json!(cwd)); }
            Value::Object(obj)
        }
        McpTransport::Http { url, headers } => {
            ...
            if !headers.is_empty() { obj.insert("headers".into(), json!(headers)); }
            Value::Object(obj)
        }
...
/// Read servers currently present in Claude Code's config (useful for import/discover).
