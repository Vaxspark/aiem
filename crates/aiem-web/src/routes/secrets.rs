...
fn form_view() -> Markup {
    card(html! {
        div class="text-sm font-semibold mb-3" { "Set secret" }
...
fn render(vault: Option<&Vault>) -> Markup {
    let Some(v) = vault else {
        return html!{ div class="aiem-card" style="color:var(--danger)" { "Failed to load vault." } };
    };
    let names: Vec<&String> = v.names().collect();
    if names.is_empty() {
        return empty_state("No secrets stored", "Add one above — values live in the OS keyring, not on disk.");
    }
    card(html! {
        table class="aiem" {
...
async fn set(State(st): State<AppState>, Form(f): Form<SetForm>) -> Response {
    ...
    let mut v = match Vault::load() { Ok(v) => v, Err(e) => { toast_error(&tx, format!("{e}")); return ok(); } };
    let desc = if f.description.is_empty() { None } else { Some(f.description) };
    match v.set(&f.name, &f.value, desc) {
        Ok(()) => { toast_info(&tx, format!("saved {}", f.name)); invalidate(&tx, ResourceKind::Secrets); }
...
async fn remove(State(st): State<AppState>, Path(name): Path<String>) -> Response {
    ...
    let mut v = match Vault::load() { Ok(v) => v, Err(e) => { toast_error(&tx, format!("{e}")); return ok(); } };
    match v.delete(&name) {
        Ok(()) => { toast_info(&tx, format!("deleted {name}")); invalidate(&tx, ResourceKind::Secrets); }
