...
use crate::layout::{btn_primary, btn_secondary, card, empty_state, page, page_header};
...
async fn index(State(_st): State<AppState>) -> Markup {
    ...
    page("Discover", "/discover", html!{
        (page_header("Discover", "Scan your IDEs for unmanaged skills & MCP servers and import them.", html!{}))
        (card(html!{
            div class="flex items-center justify-between mb-3" {
                div class="text-sm font-semibold" { "Unmanaged skills (" (skills.len()) ")" }
                @if !skills.is_empty() {
                    form hx-post="/discover/import-all-skills" hx-swap="none" { (btn_primary("Import all")) }
                }
            }
            @if skills.is_empty() {
                        ...
                        @for s in &skills {
                            tr {
                                ...
                                td style="text-align:right" {
                                    form hx-post="/discover/import-skill" hx-swap="none" {
        ...
        (card(html!{
            div class="flex items-center justify-between mb-3" {
                div class="text-sm font-semibold" { "Unmanaged MCP servers (" (mcps.len()) ")" }
                @if !mcps.is_empty() {
...
#[derive(Deserialize)]
struct ImportSkillForm { path: String, ide: String, name: String }
...
async fn import_skill(State(st): State<AppState>, Form(f): Form<ImportSkillForm>) -> Response {
    ...
    match discover::import_skill(&found, false) {
        Ok(skill) => {
...
async fn import_all_skills(State(st): State<AppState>) -> Response {
    let tx = st.events.clone();
    let _g = st.write_lock.lock().await;
    match discover::discover_skills() {
        Ok(found) => match discover::import_all_skills(&found, false) {
            Ok(n) => { toast_info(&tx, format!("imported {n} skill(s)")); invalidate(&tx, ResourceKind::Skills); }
...
#[derive(Deserialize)]
struct ImportMcpForm { name: String }
        ...
        Ok(found) => {
            if let Some(m) = found.iter().find(|m| m.server.name == f.name) {
                match discover::import_mcp(m) {
                    Ok(()) => { toast_info(&tx, format!("imported {}", f.name)); invalidate(&tx, ResourceKind::Mcp); }
                    Err(e) => toast_error(&tx, format!("{e}")),
                }
            } else {
...
async fn import_all_mcp(State(st): State<AppState>) -> Response {
    ...
    match discover::discover_mcp() {
        Ok(found) => match discover::import_all_mcp(&found) {
            Ok(n) => { toast_info(&tx, format!("imported {n} MCP server(s)")); invalidate(&tx, ResourceKind::Mcp); }
            Err(e) => toast_error(&tx, format!("{e}")),
        },
        Err(e) => toast_error(&tx, format!("{e}")),
    }
