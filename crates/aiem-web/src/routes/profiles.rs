...
async fn index(State(st): State<AppState>) -> Markup {
    page("Profiles", "/profiles", html!{
        (page_header("Profiles", "Group skills and MCP servers; switch sets of tools per task.", html!{}))
        (card(html!{
            div class="text-sm font-semibold mb-3" { "Create profile" }
...
fn render(store: Option<&ProfileStore>) -> Markup {
    ...
    if items.is_empty() {
        return empty_state("No profiles yet", "Create one above to bundle skills & MCP servers.");
    }
            ...
            tbody {
                @for p in &items {
                    ...
                    tr {
                        ...
                        td class="meta" { (p.skills.len()) " skills · " (p.mcp_servers.len()) " MCP" }
                        td {
...
#[derive(Deserialize)]
struct CreateForm { name: String, #[serde(default)] description: String }
...
async fn create(State(st): State<AppState>, Form(f): Form<CreateForm>) -> Response {
    ...
    let mut store = match ProfileStore::load() { Ok(s)=>s, Err(e)=>{toast_error(&tx,format!("{e}"));return ok();} };
    store.upsert(Profile { name: f.name.clone(), description: if f.description.is_empty(){None}else{Some(f.description)}, skills: vec![], mcp_servers: vec![] });
...
async fn activate(State(st): State<AppState>, Path(name): Path<String>) -> Response {
    ...
    let _g = st.write_lock.lock().await;
    let mut store = match ProfileStore::load() { Ok(s)=>s, Err(e)=>{toast_error(&tx,format!("{e}"));return ok();} };
    if let Err(e) = store.set_active(Some(&name)) { toast_error(&tx, format!("{e}")); return ok(); }
    let _ = store.save();
...
async fn remove(State(st): State<AppState>, Path(name): Path<String>) -> Response {
    ...
    let _g = st.write_lock.lock().await;
    let mut store = match ProfileStore::load() { Ok(s)=>s, Err(e)=>{toast_error(&tx,format!("{e}"));return ok();} };
    if let Err(e)=store.remove(&name){toast_error(&tx,format!("{e}"));return ok();}
    let _ = store.save();
